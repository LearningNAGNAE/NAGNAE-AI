...
def apply_lora(model):
    ...
    model.print_trainable_parameters()  # print_trainable_parameters 함수를 모델 메서드로 사용
    return model
...
def evaluate_model(model, test_df, tokenizer):
    ...
    with torch.no_grad():
        for _, row in test_df.iterrows():
            input_text = f"Question: {row['instruction']}\nContext: {row['input']}\nAnswer:"
            ...
            if predicted_answer.strip().lower() in row['output'].strip().lower():
                correct += 1
            total += 1
...
def main():
    # 데이터 준비
    csv_path = './app/models/job/job_fine_tuning_dataset.csv'
    train_df, val_df, test_df = prepare_data(csv_path)
    print("컬럼명 확인: ", test_df.columns)
    print("데이터 준비 완료")
    ...
    # 훈련 인자 설정
    training_args = get_training_args("./app/models/job/results")
    ...
    # 모델 저장
    model_save_path = './app/models/job/fine_tuned_qwen2_1_5b'
