...
def ollama_search(query: str, es_client, language='ko') -> str:
    ...
    template=
    f"""
    당신은 한국 대학의 전문가입니다. 제공된 컨텍스트와 데이터를 바탕으로 간결하고 정확하게 답변하세요.
...
def hybrid_search(query: str, top_k: int = 6, es_weight: float = 0.3, ollama_weight: float = 0.2, openapi_weight: float = 0.5):
    ...
    try:
        ...
        for hit in es_response['hits']['hits']:
            print(f"Score: {hit['_score']}, Source: {hit['_index']}")
            print(f"Content: {hit['_source']['content'][:3000]}...")
            print("-" * 50)

        ollama_result = ollama_search(query, es_client)
        
        combined_results = [{"source": hit["_index"], "content": hit["_source"]["content"], "score": hit["_score"] * (es_weight if hit["_index"] == "pdf_search" else openapi_weight), "metadata": hit["_source"]["metadata"]} for hit in es_response["hits"]["hits"]]
        combined_results.append({"source": "ollama", "content": ollama_result, "score": ollama_weight})
...
def initialize_langchain(language='ko'):
    global agent_executor, es_client, embeddings

    class FunctionRetriever(BaseRetriever):
        search_func: Callable = Field(default_factory=lambda: None)
        ...
        def get_relevant_documents(self, query: str) -> List[Document]:
            if self.search_func is None:
                raise ValueError("search_func is not set")
            return self.search_func(query)

        async def aget_relevant_documents(self, query: str) -> List[Document]:
            return self.get_relevant_documents(query)

    def search_func(query: str) -> List[Document]:
        results = multi_index_search(query, es_client, embeddings, ["pdf_search", "openapi_data"])
        return [Document(page_content=result) for result in results]

    retriever = FunctionRetriever(search_func=search_func)
    ...
    - 사용자 질문을 정확히 파악하고 요점만 답변하세요.
    - PDF 문서와 OpenAPI 데이터를 모두 참조하여 답변하세요.
    ...
    - 특정 대학의 전공이나 모집 인원 정보는 PDF 문서의 21페이지부터 166페이지와 OpenAPI 데이터를 참조하여 정확히 제공하세요.
    - 대학별 모집유형 정보는 PDF의 11페이지에서 20페이지를 참고하세요.
    - 대학입학전형기본사항은 PDF의 1페이지부터 10페이지를 참고하세요.
    - 불필요한 설명이나 부가 정보는 제공하지 마세요.
    - 정보가 없으면 "해당 정보를 찾을 수 없습니다."라고만 답변하세요.
    - 외국인 전형 정보를 물어보면 반드시 PDF의 '전형방법 및 전형요소' 섹션(167페이지 이후)과 OpenAPI 데이터를 참고하여 답변하세요.
    - 중복된 정보는 생략하고 한 번만 제공하세요.
    - 요청받은 정보에 대해 가능한 한 완전하고 포괄적인 답변을 제공하세요. 정보를 생략하지 말고 알고 있는 모든 관련 정보를 포함해 주세요.
    - 답변 시 정확한 정보만을 제공하세요. 불확실한 정보는 제공하지 마세요.
    - 질문에 대한 답변이 PDF 문서나 OpenAPI 데이터를 참조한 경우, 해당 자료의 페이지 번호나 데이터 출처를 정확히 기재하세요. 이를 통해 사용자에게 신뢰할 수 있는 정보를 제공하세요.
    - {language} 언어로 응답하세요.
...
def generate_final_response(query: str, hybrid_results: List[Dict], agent_executor, overall_chain, chat_history, enhanced_query: str) -> str:
    ...
    chain_output = overall_chain({"query": query, "agent_response": agent_output})
    final_response = chain_output["final_response"]
...
def query(query: Query, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # 더 자세한 오류 메시지 반환
        error_message = f"내부 서버 오류: {str(e)}. 전체 오류: {traceback.format_exc()}"
