...
def ollama_search(query: str, language='ko') -> str:
...
def hybrid_search(query: str, top_k: int = 5, es_weight: float = 0.7, ollama_weight: float = 0.3):
    ...
    try:
        ...
        ollama_result = ollama_search(query)
        ...
        combined_results = es_results + ollama_results
        combined_results.sort(key=lambda x: x.get("score", 0), reverse=True)
        
        return combined_results
    except Exception as e:
        print(f"Elasticsearch 또는 Ollama 검색 중 오류 발생: {str(e)}")
        return []
...
def summarize_text(text: str, max_tokens: int = 500) -> str:
    ...
    openai = ChatOpenAI(
        model="gpt-3.5-turbo-16k", api_key=os.getenv("OPENAI_API_KEY"), temperature=0.1,
...
def query(query: Query, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        print(f"쿼리 메소드의 지역 : {entities}");
        universities = get_universities_by_region(regions)
        print(f"쿼리 메소드의 대학교 : {entities}");
        ...
        final_response = generate_response(query.input, search_results)
